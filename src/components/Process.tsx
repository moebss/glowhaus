    ...
    <section className="py-32 bg-anthracite relative">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-20">
          ...
          <h3 className="font-serif text-4xl md:text-6xl text-light leading-tight">Dein Weg zum <span className="italic text-light/80">Perfekten Look</span></h3>
        </motion.div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-12">
          {PROCESS_STEPS.map((proc, idx) => (
            <motion.div key={proc.step} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.2 }} className="relative">
              <div className="text-6xl font-serif text-light/5 mb-6">{proc.step}</div>
